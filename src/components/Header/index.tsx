...
import {
  ...
  faClipboardList, faCalendarCheck, faListCheck,
...
export function Header() {
  ...
  useEffect(() => {
    function handleUpdate() { setPicture(localStorage.getItem("@Umanizzare:picture") || ""); }
    ...
    <aside className={styles.sidebar}>
      ...
      <nav className={styles.nav}>
        <Link to="/" className={`${styles.navLink} ${isActive("/")}`}>
          <FontAwesomeIcon icon={faHouse} className={styles.navIcon} />Início
        </Link>

        {/* ADMIN */}
        {isAdm && (<>
          <Link to="/admin/users" className={`${styles.navLink} ${isActive("/admin/users")}`}>
            <FontAwesomeIcon icon={faUserShield} className={styles.navIcon} />Painel Admin
          </Link>
          <Link to="/admin/questionnaires" className={`${styles.navLink} ${isActive("/admin/questionnaires")}`}>
            <FontAwesomeIcon icon={faClipboardList} className={styles.navIcon} />Gerenciar Questionários
          </Link>
          <Link to="/admin/tasks" className={`${styles.navLink} ${isActive("/admin/tasks")}`}>
            <FontAwesomeIcon icon={faListCheck} className={styles.navIcon} />Gerenciar Tarefas
          </Link>
        </>)}
        ...
        {(isAdm || isPsicologo) && (
          <Link to="/patients" className={`${styles.navLink} ${isActive("/patients")}`}>
            <FontAwesomeIcon icon={faUsers} className={styles.navIcon} />Pacientes
          </Link>
        )}
        ...
        {isPaciente && (<>
          <Link to="/dashboard" className={`${styles.navLink} ${isActive("/dashboard")}`}>
            <FontAwesomeIcon icon={faCalendarCheck} className={styles.navIcon} />Minha Área
          </Link>
          {isAuthenticated && (
            <Link to="/questionnaire/1" className={`${styles.navLink} ${isActive("/questionnaire/1")}`}>
              <FontAwesomeIcon icon={faClipboardList} className={styles.navIcon} />Avaliação de Risco
            </Link>
          )}
        </>)}

        <Link to="/documents" className={`${styles.navLink} ${isActive("/documents")}`}>
          <FontAwesomeIcon icon={faFileAlt} className={styles.navIcon} />Documentos
        </Link>

        {(isAdm || isPsicologo) && (
          <Link to="/reports" className={`${styles.navLink} ${isActive("/reports")}`}>
            <FontAwesomeIcon icon={faChartBar} className={styles.navIcon} />Relatórios
          </Link>
        )}
        ...
        {isAuthenticated ? (<>
          ...
          <Link to="/login" className={`${styles.navLink} ${isActive("/login")}`}>
            <FontAwesomeIcon icon={faRightToBracket} className={styles.navIcon} />Login
          </Link>
          <Link to="/register" className={`${styles.navLink} ${isActive("/register")}`}>
            <FontAwesomeIcon icon={faUserPlus} className={styles.navIcon} />Cadastro
          </Link>
        </>)}
      </nav>
    </aside>
